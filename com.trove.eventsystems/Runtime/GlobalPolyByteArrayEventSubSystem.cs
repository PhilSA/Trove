...
{
    public unsafe struct GlobalPolyByteArrayEventSubSystem<S, E>
        where S : unmanaged, IComponentData, IGlobalPolyByteArrayEventsSingleton<E> // The events singleton
        where E : unmanaged, IPolymorphicObject
    {
        ...
        public GlobalPolyByteArrayEventSubSystem(ref SystemState state, int initialStreamsCapacity, int initialEventsListCapacity)
        {
            ...
            singleton.StreamEventsManager = new GlobalPolyByteArrayStreamEventsManager<E>(
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            for (int i = 0; i < eventStreams.Length; i++)
            {
                state.Dependency = new EventTransferPolyByteArrayStreamToListJob<E>
                {
    ...
    [BurstCompile]
    public unsafe struct EventTransferPolyByteArrayStreamToListJob<P> : IJob
